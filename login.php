...
$name = stripcslashes (mysqli_real_escape_string($conn, $_POST['user']));
$pass = md5($_POST['password']);

if (empty($name)){
$msg = '<div class="alert alert-danger" role="alert" style="    text-align: center;";>ادخل اسم المستخدم او البريد الالكترونى  </div>';
}
elseif(empty($_POST['password'])){
   $msg =  '<div class="alert alert-danger" role="alert">ادخل كلمة المرور </div>';

}
else {

    $sql = mysqli_query ($conn, "SELECT * FROM accounts_user WHERE ( named = '$name' OR phone = '$name') AND  user_password = '$pass'");
    if(mysqli_num_rows($sql) != 1){

        $msg =  '<div class="alert alert-danger" role="alert"> رقم الجوال او كلمة المرور غير صحيحة  </div>';      

    }else {
        
        $user = mysqli_fetch_assoc($sql);
        $_SESSION ['id'] = $user ['id_u'];
        $_SESSION['name'] = $user['named'] ;
        $_SESSION['email'] = $user['phone'] ;
        $_SESSION['pass'] = $user['user_password'] ;


        $msg =  '<div class="alert alert-success" role="alert">  تم تسجيل دخولك , جارى تحديث الصفحة </div>';
        $msg= '<meta http-equiv ="refresh" content = "0; shop-grid.php " />';
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="account-login-inner">
                        <form action="#" class="ltn__form-box contact-form-box" method="post">
                            <input type="text" name="user" placeholder="رقم الجوال">
                            <input type="password" name="password" placeholder="كلمة المرور">
                            <div class="btn-wrapper mt-0">
                                <button class="theme-btn-1 btn btn-block" name="enter_user" type="submit">تسجيل دخول</button>
                            </div>
                            <?php  echo $msg;   ?> 
                ...
                <div class="col-lg-6">
                    <div class="account-create text-center pt-50">
                        <h4>سجل في متجر الغيث في ثواني</h4>
                        <div class="btn-wrapper">
                            <a href="register.html" class="theme-btn-1 btn black-btn">انشاء حساب</a>
                        </div>
                    </div>
                </div>
            </div>
